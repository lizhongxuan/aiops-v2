...
3. Use lower-level tools only for progressive drill-down:
   ...
   - `coroot.rca_report` only as optional reference evidence, never as the primary RCA source.
...
5. Output only the sections the user needs for the current context. If context is missing, ask for the minimum missing fields instead of forcing a Root Cause / Evidence / Impact / Next Step template.
6. Cite rawRefs, evidence ids, toolCallIds, or metric/event names for every non-obvious claim.
7. If evidence is insufficient, say the result is inconclusive and list the smallest missing evidence set.
8. Keep the final text short: strongest hypothesis, confidence reason, key evidence, contradictions, and the next read-only drill-down.
