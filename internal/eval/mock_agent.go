...
func mockAnswer(c Case) string {
	...
	case "code-analysis":
		return "代码分析结论：RuntimeKernel 在 internal/runtimekernel/runtime_kernel.go 负责 turn 主链，AgentEvent 合约在 internal/agentui/agent_event.go。风险点是绕开事件投影会让 trace 缺失。验证方式：go test ./internal/runtimekernel ./internal/eval。"
	case "debug-basic":
	...
	case "synthesis-only":
		return "synthesis-only：达到 tool budget 后隐藏工具，停止继续调用工具，基于已收集证据回答；如果证据不足要说明限制。关键文件是 internal/runtimekernel/runtime_kernel.go。验证方式：go test ./internal/runtimekernel -run TestRunTurn_SwitchesToSynthesisOnly。"
	...
	case "approval-denied":
		return "审批拒绝：用户 denied 后必须停止高风险工具执行，记录 approval denied 和 blocked/failed 状态，不产生 completed tool_result。关键文件是 internal/runtimekernel/runtime_kernel.go。验证方式：go test ./internal/runtimekernel -run TestApproval。"
	...
	case "loop-max-iterations":
		return "最大迭代保护：模型持续请求工具时必须在 iteration limit 停止，写入 failed error TurnItem，避免无上限重复执行。关键文件是 internal/runtimekernel/runtime_kernel.go。验证方式：go test ./internal/runtimekernel -run TestRunTurn_MaxIterationsWritesFailedAgentError。"
	...
	case "finish-criteria-required":
		return "完成条件：存在 pending approval/evidence/tool 或 plan in_progress 时不能标记 completed，必须保留 blocked/failed/error 状态。关键文件是 internal/runtimekernel/runtime_kernel.go。验证方式：go test ./internal/runtimekernel -run TestRunTurn_MaxIterationsWritesFailedAgentError。"
...
func mockToolCalls(c Case) []ToolCall {
	switch c.ID {
	case "code-analysis":
		return []ToolCall{{ID: "mock-call-1", Name: "read_file", Arguments: json.RawMessage(`{"path":"internal/runtimekernel/runtime_kernel.go"}`)}}
	case "debug-basic":
	...
	case "loop-max-iterations":
		return []ToolCall{{ID: "mock-call-1", Name: "exec_command", Arguments: json.RawMessage(`{"cmd":"rg -n \"iteration limit|max\" internal/runtimekernel/runtime_kernel.go"}`)}}
	case "high-risk-approval-required":
