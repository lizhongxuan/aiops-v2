...
func finalStreamState(status AiopsTransportFinalStatus) string {
	if status == AiopsTransportFinalStatusRunning {
		return "streaming"
	}
	if status == AiopsTransportFinalStatusFailed || status == AiopsTransportFinalStatusCancelled {
		return "incomplete"
	}
	return "complete"
}
