...
func buildApprovals(pending []runtimekernel.PendingApproval) []ApprovalView {
	...
	for _, approval := range pending {
		approvals = append(approvals, ApprovalView{
			...
			ArgumentsHash:          strings.TrimSpace(approval.ArgumentsHash),
			ToolSurfaceFingerprint: strings.TrimSpace(approval.ToolSurfaceFingerprint),
			PermissionSnapshotHash: strings.TrimSpace(approval.PermissionSnapshotHash),
			PostCheck:              strings.TrimSpace(approval.PostCheck),
			StopCondition:          strings.TrimSpace(approval.StopCondition),
			ManualTakeover:         strings.TrimSpace(approval.ManualTakeover),
