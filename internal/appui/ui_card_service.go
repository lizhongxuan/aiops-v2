...
func defaultUICardDefinitionsWithPluginSpecs(pluginSpecs []plugins.Spec) []store.UICard {
	...
	specs := []struct {
		...
		{"workflow-context", "workflow_context", "Workflow Context", "Shows Workflow AI editor context.", []any{"open_workflow"}, map[string]any{"id": "sample-workflow-context", "type": "workflow_context", "payload": map[string]any{"workflowId": "workflow-demo"}}},
		{"workflow-edit-plan", "workflow_edit_plan", "Workflow Edit Plan", "Shows Workflow AI edit plan before patching.", []any{"review_plan"}, map[string]any{"id": "sample-workflow-edit-plan", "type": "workflow_edit_plan", "payload": map[string]any{"planId": "plan-demo"}}},
		{"workflow-patch-preview", "workflow_patch_preview", "Workflow Patch Preview", "Shows one proposed Workflow patch and affected nodes.", []any{"apply_patch", "reject_patch"}, map[string]any{"id": "sample-workflow-patch-preview", "type": "workflow_patch_preview", "payload": map[string]any{"patchId": "patch-demo"}}},
		{"workflow-patch-apply", "workflow_patch_apply", "Workflow Patch Apply", "Shows user-confirmed Workflow patch application.", []any{"confirm_apply"}, map[string]any{"id": "sample-workflow-patch-apply", "type": "workflow_patch_apply", "payload": map[string]any{"patchId": "patch-demo"}}},
		{"workflow-patch-result", "workflow_patch_result", "Workflow Patch Result", "Shows applied Workflow patch result, effect and undo checkpoint.", []any{"undo_patch"}, map[string]any{"id": "sample-workflow-patch-result", "type": "workflow_patch_result", "payload": map[string]any{"effectStatus": "changed"}}},
		{"workflow-patch-validation", "workflow_patch_validation", "Workflow Patch Validation", "Shows Workflow patch validation output.", []any{"review_validation"}, map[string]any{"id": "sample-workflow-patch-validation", "type": "workflow_patch_validation", "payload": map[string]any{"valid": true}}},
		{"workflow-conflict", "workflow_conflict", "Workflow Conflict", "Shows Workflow AI revision or undo conflict.", []any{"reload_snapshot"}, map[string]any{"id": "sample-workflow-conflict", "type": "workflow_conflict", "payload": map[string]any{"reason": "stale_revision"}}},
		{"workflow-manual-candidate", "workflow_manual_candidate", "Workflow Manual Candidate", "Shows pending OpsManual candidate from Workflow AI.", []any{"save_candidate", "open_review"}, map[string]any{"id": "sample-workflow-manual-candidate", "type": "workflow_manual_candidate", "payload": map[string]any{"reviewStatus": "pending"}}},
		{"workflow-tool-timeline", "workflow_tool_timeline", "Workflow Tool Timeline", "Shows Workflow AI tool calls and traces.", []any{"copy_log"}, map[string]any{"id": "sample-workflow-tool-timeline", "type": "workflow_tool_timeline", "payload": map[string]any{"tool": "workflow.describe"}}},
