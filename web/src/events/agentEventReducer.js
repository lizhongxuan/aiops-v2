...
function applySystem(projection, event) {
  ...
  const row = {
    ...
    runbookId: compactText(event.payload.runbookId),
    runbookStep: compactText(event.payload.runbookStep),
    expectedEffect: compactText(event.payload.expectedEffect),
    rollback: compactText(event.payload.rollback),
