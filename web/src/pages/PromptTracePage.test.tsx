...
describe("PromptTracePage", () => {
  ...
  it("keeps Prompt Trace lists compact and opens LLM details in a dialog", async () => {
    ...
    const detailDialog = document.body.querySelector('[data-testid="prompt-trace-detail-dialog"]');
    expect(detailDialog?.textContent).toContain("模型请求详情");
    expect(detailDialog?.className).toContain("max-h-[calc(100vh-3rem)]");
    expect(detailDialog?.className).toContain("flex");
    expect(detailDialog?.className).toContain("flex-col");
    expect(detailDialog?.className).toContain("overflow-hidden");
    expect(document.body.querySelector('[data-testid="prompt-trace-detail-scroll"]')?.className).toContain("min-h-0");
    expect(document.body.querySelector('[data-testid="prompt-trace-detail-scroll"]')?.className).toContain("flex-1");
    expect(document.body.querySelector('[data-testid="prompt-trace-detail-scroll"]')?.className).toContain("overflow-y-auto");
    expect(detailDialog?.textContent).not.toContain("稳定哈希");
    expect(detailDialog?.textContent).not.toContain("开发者规则");
    expect(detailDialog?.textContent).not.toContain("工具表 tools-hash");
    expect(detailDialog?.textContent).not.toContain("Provider");
    expect(detailDialog?.textContent).not.toContain("Cache");
    ...
    expect(document.body.textContent).not.toContain("文本输出");
    expect(document.body.textContent).toContain("模型工具调用");
    ...
    expect(document.body.textContent).toContain("平均响应456ms");
    expect(document.body.textContent).not.toContain("456 ms");
    ...
    expect(document.body.textContent).not.toContain("输入 21 / 输出 8 / 总计 29");
    expect(document.body.querySelector('[data-testid="prompt-trace-llm-tool-calls"]')?.textContent).toContain("coroot.query_latency");
