...
import { nodeTypeLabel, topologyNodeMeta, visibleTopologyNodes } from "./opsGraphViewModel";

export function OpsGraphNodeList({ nodes }: { nodes: OpsGraphNode[] }) {
  const [query, setQuery] = useState("");
  const topologyNodes = useMemo(() => visibleTopologyNodes(nodes), [nodes]);
  const filteredNodes = useMemo(() => {
    const normalizedQuery = query.trim().toLowerCase();
    if (!normalizedQuery) return topologyNodes;
  ...
  return (
    <section data-testid="opsgraph-node-list" className="grid min-h-0 grid-rows-[auto_auto_minmax(0,1fr)] gap-2">
      <div>
        ...
        <p className="mt-1 text-xs leading-5 text-slate-500">{topologyNodes.length} 个节点</p>
      </div>
        ...
        {filteredNodes.length ? filteredNodes.map((node) => {
          ...
          return (
            <button key={node.id} type="button" className="rounded-lg border bg-white p-2 text-left text-sm hover:bg-slate-50">
              ...
              <span className="mt-1 block text-xs text-slate-500">{meta.typeLabel}</span>
              {meta.summary ? <span className="mt-1 block truncate text-xs text-slate-500">{meta.summary}</span> : null}
            </button>
          );
        }) : (
          <p className="rounded-lg border border-dashed bg-slate-50 p-3 text-sm leading-6 text-slate-500">{topologyNodes.length ? "没有匹配的节点。" : "还没有节点。"}</p>
        )}
      </div>
    </section>
  );
