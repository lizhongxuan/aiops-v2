...
const incidentPayload = {
  ...
  host_profile_snapshots: [{ host_id: "host-web-1", display_name: "web-1", labels: { env: "prod", role: "web" } }],
  host_leases: [{ lease_id: "lease-1", host_id: "host-web-1", status: "acquired", expires_at: "2026-05-12T10:00:00+08:00" }],
  workflow_runs: [{ run_id: "run-1", workflow_id: "wf-checkout-fix", status: "succeeded", verification_refs: ["verify-1"] }],
  verifications: [{ id: "verify-1", title: "Latency recovered", status: "passed", summary: "p95 back to baseline" }],
  experience_candidates: [{ pack_id: "pack-checkout-lock", title: "Checkout lock wait pack", status: "candidate" }],
...
const slowButtonIncidentPayload = {
  ...
  pendingActions: [{ actionId: "confirm-slow-button", title: "确认慢按钮修复", status: "pending" }],
  debug_event: {
...
const pgFixIncidentPayload = {
  ...
  host_leases: [{ lease_id: "lease-pg-conflict", host_id: "db-pg-1", status: "conflict" }],
  pendingActions: [{ actionId: "confirm-pg-fix", title: "确认 PG 修复", status: "pending" }],
  pendingApprovals: [{ id: "approval-pg-conflict", command: "runner workflow run pg-pool-fix", decision: "pending" }],
};
...
const opsGraphLookupPayload = {
  matches: [{ id: "pg-primary", name: "PG 主库", type: "middleware", status: "warning" }],
};
...
function mockFetch(input: RequestInfo | URL, init?: RequestInit) {
  ...
  if (url.includes("/api/v1/incidents/case-slow-button")) return jsonResponse(slowButtonIncidentPayload);
  if (url.includes("/api/v1/incidents/case-pg-fix")) return jsonResponse(pgFixIncidentPayload);
  if (url.includes("/api/v1/incidents")) {
    const params = new URL(url, "http://localhost").searchParams;
    if (params.get("lock_conflict") === "true") return jsonResponse({ items: [pgFixIncidentPayload] });
...
describe("React complex migration pages", () => {
  ...
  it.each([
    ...
    ["/runbooks/checkout-restart", "Runbook 已不作为当前方案主概念，请使用 Runner Workflow"],
    ["/mcp", "docs"],
  ...
  it("moves complex page actions into the app shell header", async () => {
    ...
    expect(container.querySelector("main > div header")?.textContent || "").not.toContain("展示 Debug Mode");

    act(() => root.unmount());
    container.innerHTML = "";
    root = createRoot(container);
    await render("/mcp");
    const mcpHeader = container.querySelector('[data-testid="app-shell-header"]');
    ...
    expect(container.querySelector("main > div header")?.textContent || "").not.toContain("MCP runtime server");
  });

  it.each(["/runbooks", "/runbooks/checkout-restart"])("keeps %s as a Runner Workflow compatibility route", async (path) => {
    await render(path);

    expect(container.textContent).toContain("Runbook 已不作为当前方案主概念，请使用 Runner Workflow");
    const runnerLinks = Array.from(container.querySelectorAll('a[href="/runner"]'));
    expect(runnerLinks.some((link) => link.textContent?.includes("前往 Runner Workflow"))).toBe(true);
    const runbookApiCalls = vi.mocked(globalThis.fetch).mock.calls.filter(([input]) => String(input).startsWith("/api/v1/runbooks"));
  ...
  it("renders Case workbench closure context from a legacy incident payload", async () => {
    ...
    expect(container.querySelector('a[href="/debug/prompts?case_id=incident-1"]')?.textContent).toContain("Prompt Trace");
  });
  ...
  it("applies Case list filters for status, source, environment, host and blockers", async () => {
    ...
    const filtersPanel = container.querySelector('[data-testid="case-list-filters"]');
    ...
    expect(container.querySelector('a[href="/incidents/case-slow-button"]')?.textContent).toContain("页面按钮很慢");
    expect(container.querySelector('a[href="/incidents/case-pg-fix"]')?.textContent).toContain("PG 锁冲突修复");

    const status = container.querySelector('[aria-label="状态筛选"]') as HTMLSelectElement | null;
    const source = container.querySelector('[aria-label="来源筛选"]') as HTMLSelectElement | null;
    const environment = container.querySelector('[aria-label="环境筛选"]') as HTMLSelectElement | null;
    const host = container.querySelector('[aria-label="主机筛选"]') as HTMLInputElement | null;
    const waitingConfirmation = container.querySelector('[aria-label="待确认筛选"]') as HTMLSelectElement | null;
    const lockConflict = container.querySelector('[aria-label="锁冲突筛选"]') as HTMLSelectElement | null;
    ...
    const apply = Array.from(container.querySelectorAll("button")).find((button) => button.textContent?.includes("应用筛选"));
  ...
  it("explains how Cases and OpsGraph relationships are created when data is empty", async () => {
    vi.mocked(globalThis.fetch).mockImplementation((input, init) => {
      const url = String(input);
      if (url.includes("/api/v1/incidents") && !url.includes("/api/v1/incidents/")) {
  ...
  it("does not allow approving workflow execution while HostLease is blocked", async () => {
    ...
    const approve = Array.from(container.querySelectorAll("button")).find((button) => button.textContent?.includes("批准"));
    const reject = Array.from(container.querySelectorAll("button")).find((button) => button.textContent?.includes("拒绝"));
    expect(approve).toBeTruthy();
