...
Core 代码不得继续用固定字符串表来判断用户意图、证据类型、运维领域、工具面或最终回答完整性。`looksLike*`、`userRequests*`、`containsAny(...)`、`strings.Contains(lower, "...")` 这类规则如果服务的是业务语义，而不是协议解析或安全边界，默认视为场景补丁，不能进入 `appui` route、`runtimekernel`、`promptcompiler`、`tooling`、`policyengine`、`opsmanual` 主路径。

- 禁止用关键词命中决定 `chat_advisory` / `evidence_rca` / `host_bound_ops`、`AllowsWebLearn`、`AllowsExecCommand`、`taskDepth`、tool pack 可见性、ops manual 可执行性或 final gate 是否通过。
- 禁止为了某个样例加入类似“看到 timeline / checkpoint / pg_controldata 就按 PostgreSQL RCA 处理”“看到 最新 / 官方 / 文档 就打开公网搜索”“看到 不要执行 就只靠字符串关闭执行面”的 core 分支。用户语言只能作为模型输入和 trace 证据，不能单独成为 hard decision。
- 允许的固定规则只限结构化边界：URL/path/host mention/secret/credential 格式识别、JSON/YAML/schema/version 解析、明确协议字段、命令安全 allow/deny、sandbox/approval/policy、敏感信息脱敏、测试 fixture 断言。即使这些规则使用字符串，也必须处理的是机器边界，不是运维业务语义。
- 必须识别业务语义时，优先产出结构化 `IntentFrame` / `OperationFrame` / `EvidenceEnvelope`，由模型、工具 schema、能力包 metadata、检索结果和运行时事实共同给出 `confidence`、`source`、`evidence_spans`；规则只能作为弱信号进入 trace，不能直接打开工具或执行路径。
- 如果确实需要启发式，必须满足四个条件：放在对应 plugin/capability pack/adapter 内；输出是候选 signal 而不是最终决策；有同类正例、反例、跨产品 eval 覆盖；review 中说明为什么不能用 schema、parser、tool metadata、模型结构化分类或 policy 替代。
- Code review 必须拒绝新增“样例关键词 -> 能力路径”的补丁。正确做法是扩展通用 contract、工具 metadata、能力包、eval 用例或 prompt 片段，让模型和运行时在统一边界内处理同类问题。
- 提交前必须运行 `bash scripts/check-no-core-string-semantics.sh`。该检查用于阻止 `appui` route、`runtimekernel`、`tooling`、`taskdepth` 继续引入场景补丁式字符串语义规则。
- Agent Runtime 上下文瘦身或单路径收敛改动还必须运行 `bash scripts/check-aiops-agent-runtime-context-single-path.sh`。该检查用于阻止 HostOps synthetic completed turn、approval fallback new turn、双 dynamic prompt 注入、旧 developer core、legacy progress fallback 和 hidden-but-callable 工具语义继续残留。

### 2. AI Chat Agent Runtime 设计规则

AI Chat 必须学习 Codex 的单一 turn loop、短 progress、工具时间线和稳定 final 设计，但不能复制 Codex 的代码路径或把运维场景做成补丁式分支。

- 只有复杂主机运维任务才启动主机 Agent。用户只是要求在某个 `@主机` 上执行一条明确 `exec` 命令时，AI Chat 当前 turn 可以直接调用受治理 `exec_command`，不需要创建 host-bound child agent。
- 主机 Agent 虽然不直接显示为 AI Chat 对话主体，但运行逻辑必须和 AI Chat 上的 agent runtime 一致：同样进入 `RunTurn` / `ResumeTurn`，同样使用 tool registry、prompt compiler、policy、permission、approval、checkpoint、tool result 和 final gate。
- AI Chat 判断运维任务复杂时，可以像 `codex/` 一样进入 plan 机制，先规划运维步骤、风险、证据和分派，再把明确子任务发送给对应主机 Agent；不能绕过 plan 直接生成后台 mission completed turn。
- 一个主机最多对应一个 host-bound child agent。主机 Agent 的命令仍必须回到 AI Chat 审批；审批 UI 和 runtime trace 必须显示 host identity、command、risk、scope、approval id 和 resume target。
- 对话过程必须保留 `assistant_message phase=commentary`，并严格按真实事件顺序投影，不能把所有 tool call 聚合到过程末尾。对话结束时，终态长回答必须归入 `assistant_message phase=final_answer`；commentary 中只保留 1-2 条短说明、关键转向、阻塞或下一步，不承载完整 RCA 长文。
- 必须继续遵守“禁止场景补丁式字符串语义规则”。本节所有规则都必须通过结构化 mention、tool metadata、runtime state、OperationFrame、EvidenceEnvelope、policy 和 trace 实现，不能新增按产品名、故障名、样例文本或关键词触发的 core 分支。
- AI Chat 需要保留 Codex-like 的同类网页搜索和同类命令折叠汇总：原始时间线顺序不变，但同类 `web_search` / page open / find 或同类命令可以折叠成一组，避免刷屏。普通工具发现、MCP 调用、审批、artifact、用户可见证据不能全部压成一个“已调用 N 个工具”。
- 无显式 mention 时不得注入对应环境上下文或工具面：没有 `@ip` / `@主机`，不得要求、读取或连接任何主机；没有 `@ops_graph`，整个 agent runtime 上下文和 model-visible tools 中不得出现 OpsGraph 工具；没有 `@coroot`，不得出现 Coroot 工具；没有 `@ops_manus`，不得出现 `search_ops_manuals` 工具。`@ip` / `@主机`、`@ops_graph`、`@coroot`、`@ops_manus` 必须在 AI Chat 对话框和输入框中作为特殊 mention 处理，而不是普通关键词。
- Codex 类 RCA 交互的输出边界必须固定：模型可以先发 1-2 条短 commentary 说明分析方向，然后执行搜索、打开页面、页内查找或命令；最终 RCA 必须作为 `assistant_message phase=final_answer` 输出，原因段落直接带官方文档或工具证据的内联引用。不要把模型长输出当作 `phase=commentary`，不要让大段分析挂在过程区。
- 修复 commentary/final 归类要做成通用投影链路：后端投影负责把 `assistant_message phase=final_answer` 稳定归到 final 区；prompt 只负责约束模型不要把完整 RCA 写进 commentary，并要求网页来源以内联引用挂到结论里。不能为 pgBackRest、Redis、Coroot 或任何单一问题打专项补丁。
- 模型超时必须有容错机制。不能因为一次模型调用超时就直接把 turn 作为硬错误结束；runtime 应保留已有 `assistant_message`、tool result、pending approval/evidence 和 checkpoint，给出可恢复、可重试或可降级的状态，并参考 `codex/` 的 retry/resume/compact 思路继续同一 turn。
...
本地排查 AI Chat 模型慢时必须使用 `./scripts/start-ai-chat-trace-dev.sh`。启动后每次真实 LLM 调用都应在 `.data/model-input-traces/<session>/<turn>/` 下生成 trace；如果 Chat 页面变慢但 Prompt Trace 没有最新文件，优先检查 `AIOPS_DEBUG_MODEL_INPUT_TRACE` 和 `AIOPS_DEBUG_MODEL_INPUT_TRACE_DIR` 启动环境变量。
