...
## 4. Visual Workflow（生产级 DAG 编排）

Runner 支持生产级可视化编排：Graph 是唯一长期执行语义，旧 `steps[]` YAML 会作为 graph 的线性兼容形态继续可用。

能力范围：
- 可视化 DAG：顺序、条件、并行、join、失败分支、人工审批、子流程。
- YAML 兼容：旧 workflow 可自动转换为 graph，保存后保留 `x_runner_ui` 和 `x_runner_graph`。
- 运行态闭环：SSE、run graph overlay、失败节点聚焦、审批操作、日志和导出变量。
- 生产治理：版本、发布门禁、dry-run、风险确认、审计、权限和指标。

相关文档：
- [Graph YAML 规范](docs/visual-workflow-graph-yaml.md)
- [Visual Workflow API 文档](docs/visual-workflow-api.md)
- [用户操作 runbook](docs/visual-workflow-runbook.md)
- [ActionSpec 扩展指南](docs/action-spec-extension.md)
- [旧 workflow 自动迁移说明](docs/legacy-workflow-migration.md)
- [生产发布检查表](docs/visual-workflow-release-checklist.md)
