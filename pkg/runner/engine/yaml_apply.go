...
// ApplyYAML executes workflow YAML directly with the default engine registry.
...
// ApplyYAMLWithRun performs load, parameter injection, validation, and execution.
...
func buildDispatcherFromParams(params map[string]any) (scheduler.Dispatcher, error) {
	...
	dispatcher := scheduler.NewAgentDispatcher(readStringParam(dispatchMap, "base_url"))
	dispatcher.Token = readStringParam(dispatchMap, "token")
