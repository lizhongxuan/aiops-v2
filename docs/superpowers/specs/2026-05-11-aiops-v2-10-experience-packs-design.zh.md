# AIOps Evolution Map 与经验包自我演化设计方案
...
- 用户侧慢请求调试和中间件自治修复需要成为新的经验来源：前者把一次按钮点击到后端链路的 trace 转成 DebugCase，后者把一次 PG/Redis/MQ 等修复转成 OperationCase 或 IncidentCase。
...
因此本方案不另起一套执行系统，而是在现有 Incident、Runbook、Runner Workflow、Approval、Audit 之上新增“Evolution Map 层”“经验包审核层”和“环境变体层”。

## 2. 设计原则

1. 候选和发布强隔离：自动生成的内容只能进入 `experience_candidate`，不能直接进入 Runbook 目录或 Workflow 列表。
2. 发布资产不可变：已发布 Runbook / Workflow 不原地自改，任何演化都生成新版本或环境变体。
3. 证据驱动：经验包必须保存来源、适用环境、操作结果、验证结果、失败原因和审核记录。
4. 环境优先匹配：匹配 Runbook / Workflow 前先匹配环境画像，避免 Linux、macOS、Kubernetes、不同发行版之间误用。
5. 高风险动作人工兜底：涉及重启、写文件、扩缩容、配置变更、数据修复的候选必须人工审核和 dry-run。
6. 与现有门禁复用：Workflow 继续使用现有 validate、dry-run、publish、risk acknowledgement、audit 机制。
7. AI Chat 推荐强门禁：未审核 Gene、Capsule、Debug RCA、RepairPlan、经验包、Runbook draft、Workflow draft 不能进入 PromptCompiler、检索排序、推荐提示或自动建议上下文。
8. 先观察再固化：系统可以自动记录和生成候选，但不能把候选当成最佳实践；只有经过 reviewer 批准并写入 Activation Index 的资产才能影响下一次对话。

## 3. 已确认架构

本设计采用 `Evolution Map + Experience Pack` 作为上游资产架构。

系统自动生成“经验包候选”，并把每次 AI Chat、用户侧 Debug Trace、Coroot 证据、中间件修复、工具调用、审批、Workflow 运行和结果验证写入 Evolution Map。经验包可包含 Gene candidate、Capsule candidate、Debug RCA candidate、RepairPlan draft、Runbook draft、Workflow graph draft、环境画像、证据链、审核记录和版本 lineage。人工审核后，系统按审核决定激活 Gene/Capsule/Debug RCA/RepairPlan，或发布到 Runbook、Workflow。

这个架构把“对话推荐”“知识沉淀”和“可执行自动化”解耦：Evolution Map 负责长期演化关系和结果反馈，Experience Pack 负责候选资产审核，Activation Index 负责已审核经验进入 AI Chat，Runbook / Workflow 负责最终固化和执行。

## 4. 核心概念

### Evolution Map

Evolution Map 是 aiops-v2 在目标环境里的运维经验演化图。它不是单纯的知识库，而是一张带证据、环境、审核和结果反馈的关系图。
...
- `activated`：一次 Chat 使用了哪个已审核 Gene/Capsule/Debug RCA/RepairPlan。
...
Gene 是小而稳定的运维判断规则，只影响 AI Chat 的推理和推荐，不直接执行动作。
...
只有 `status=approved` 且写入 Activation Index 的 Gene 才能被 AI Chat 检索和推荐。
...
- Gene Candidate：面向 AI Chat 的小粒度推荐规则。
- Capsule Candidate：面向场景的完整处置模式。
...
Activation Index 是 AI Chat、DebugCase 和中间件修复流程唯一可用的经验检索入口，只包含人工审核通过的 Gene、Capsule、Debug RCA、RepairPlan、Runbook 和 Workflow metadata。Candidate Store 和 Activation Index 必须物理或逻辑隔离。
...
```text
Coroot / Frontend Debug / Trace / Middleware Repair / Chat / Tool / Workflow / Approval / Postmortem / Outcome
        |
        v
...
Candidate Store  ->  Review Queue
        ...
        |                  |
        v                  v
Evolution Map      Activation Index + Runbook / Workflow
        |
        v
...
4. 前端 Debug SDK 和 gateway/backend tracing 提供 trace id 贯通能力。
...
2. 从 Activation Index 检索已审核 Gene/Capsule/Debug RCA/RepairPlan/Runbook/Workflow。
3. 检查环境兼容性和禁用条件。
...
7. 给出计划：说明推荐来源、适用环境、风险、验证项、回滚方式。
...
- 成功且可复用的判断，生成 Gene candidate。
- 成功且步骤完整的处置，生成 Capsule candidate。
...
- 批准 Gene：写入 Activation Index，可影响 AI Chat 推荐。
- 批准 Capsule：写入 Activation Index，可被 Chat 用作处置模式。
...
下一次相似问题出现时，AI Chat 会优先激活已审核、环境兼容、fitness 更高的 Gene/Capsule/Debug RCA/RepairPlan。新的执行结果继续回写 Evolution Map，形成：

```text
更多真实运维与调试修复 -> 更多证据 -> 更准确的 Gene/Capsule/RepairPlan -> 更好的推荐和 Workflow
...
- Frontend Debug：页面、路由、用户动作、trace id、前端耗时、接口耗时、脱敏 headers 摘要。
...
- `activated_assets`：本次 AI Chat 使用了哪些已审核 Gene/Capsule/Debug RCA/RepairPlan/Runbook/Workflow。

相似 case 会聚合到同一个经验包候选，避免每次运行都生成一个新包。

### 7.3 Candidate Synthesizer

Synthesizer 只生成候选，不发布。
...
- Gene diff：新增了哪些推荐规则、触发条件、禁用条件和证据。
- Capsule diff：新增了哪些完整处置模式、验证项和回滚。
...
- `approve_gene`：写入 Activation Index，允许影响 AI Chat 推荐。
- `approve_capsule`：写入 Activation Index，允许作为 Chat 处置模式。
...
- Gene/Capsule/Debug RCA/RepairPlan 只有人工审核通过后才能进入 Activation Index。
...
每个已审核 Gene/Capsule/Debug RCA/RepairPlan/Runbook/Workflow 都维护 fitness，用于推荐排序和演化判断。
...
- reviewer 可以手动 pause 一个 Gene/Capsule/Debug RCA/RepairPlan，使其暂时不影响 Chat 推荐。
...
- “命中 Gene：nginx reload 前置检查，适用 linux/systemd，成功率 91%。”
- “命中 Capsule：Nginx reload recovery v3，适用 checkout namespace。”
...
业务页面的 Debug Mode 结束后，事故工作台和经验包库应提供“从本次 Debug Trace 生成经验候选”入口。候选详情展示页面动作、trace id、慢 span、Coroot RCA、服务路径、建议修复动作和脱敏状态。
...
发布弹窗显示“此 workflow 来自经验包候选”，并展示审核人、证据链和环境适用范围。
...
- Frontend Debug / Trace：接入 DebugEvent、TraceContext、W3C traceparent、前端耗时和 trace backend span 摘要。
...
- Runner Workflow：复用 graph draft、validate、dry-run、publish、history 和 hash。
- Approval/Audit：候选生成、审核、发布都记录 audit；高风险动作继续走现有审批。
- PromptCompiler：只读取 Activation Index，不读取 Candidate Store。
- RuntimeKernel：每次 Chat 写入 OperationEvent，记录 activated asset 和 outcome。
...
### Phase 3：Gene/Capsule/Debug RCA/RepairPlan 人工激活

- 生成 Gene/Capsule candidate。
- 生成 Debug RCA candidate 和 RepairPlan draft。
- 增加审核动作 `approve_gene`、`approve_capsule`、`approve_debug_rca`、`approve_repair_plan`。
...
- Runner Studio 展示来源、证据链、环境范围和 diff。
- 发布后写入 lineage。
...
- 用户侧 Debug Mode 产生慢请求 trace 后，系统能生成 Debug RCA candidate，包含页面动作、trace id、慢 span、Coroot RCA、服务路径、建议修复和脱敏状态。
...
- 未审核 Gene/Capsule/Debug RCA/RepairPlan 不会进入 PromptCompiler 输入、Activation Index 或推荐排序。
- 审核通过的 Gene/Capsule/Debug RCA/RepairPlan 会在下一次相似 AI Chat、DebugCase 或中间件修复 case 中被展示为推荐来源。
- 人工审核通过后，Runbook 才能被 `/api/v1/runbooks` 返回。
- 从经验包生成的 workflow draft 必须能在 Runner Studio 中 validate 和 dry-run。
