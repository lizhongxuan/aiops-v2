...
前端设计：[2026-05-11-aiops-v2-08a-middleware-repair-frontend-design.zh.md](2026-05-11-aiops-v2-08a-middleware-repair-frontend-design.zh.md)
...
本模块先把 PG 集群修复做完整：资源识别、只读 RCA、经验匹配、`RepairPlan`、用户确认、受治理执行、验证和经验候选。Redis、MQ、Elasticsearch、Kafka、MySQL 等先沿用对象模型和只读诊断，后续再扩展深度修复。

本模块不要求改现有 AI Chat 页面，也不要求改现有 Runner 工作流页面或功能。用户仍在现有 AI Chat 输入“帮我修复 xxx 的 PG 集群”；后端识别意图后创建 `middleware_repair` case，后续执行复用现有 Runner workflow 引擎、ToolDispatcher、审批和事件能力。

默认不自动执行 failover、删数据、强制 promote、清理 WAL 或数据修复。没有备份、回滚和验证证据时，只能停在诊断或计划阶段。
...
- 优先复用已审核经验包、Skill、Runbook 或 Workflow。
- 未命中经验时生成 RepairPlan 草稿，用户确认后才执行。
- 中间件修复必须比普通服务修复更严格地处理数据安全、角色、复制、备份和回滚。
- 成功和失败都沉淀为经验包候选。
- 复用现有 AI Chat 和 Runner workflow 能力，不新增普通用户入口。
...
```text
User request
  -> existing AI Chat backend recognizes middleware_repair intent
  ...
  -> execute with existing Runner engine / ToolDispatcher under Approval
...
- 经验来源 case。
- Skill / RepairPlan / Runbook / Workflow 绑定。
- 适用环境。
- 成功率和最近失败。
- 禁用条件。
- 所需权限。
...
- 现有 AI Chat 页面不需要新增专用入口；后端能识别 PG 修复意图并进入 middleware_repair flow。
- 修复执行复用现有 Runner workflow 引擎和 ToolDispatcher，不要求改 Runner 页面或功能。
