...
告警 / 用户问题 / ERP 业务异常
  -> IncidentCase
  -> Evidence / OpsGraph / Business Impact
...
所有生产运维行为都应尽量归属到一个 `IncidentCase` 或一个明确的 `OperationCase`。聊天、Coroot、Runbook、Workflow、审批、主机锁、工具结果、复盘、经验包都围绕 case 组织。
...
```mermaid
flowchart TD
  Inputs["ERP / Coroot / User / Ticket / Change / Logs / Traces"] --> ICP["Incident Control Plane"]
...
- 把 Coroot webhook、用户问题、ERP 异常、工单事件统一映射为 case。
...
Case
  id
  type: incident | operation | maintenance | drill
  status
  severity
  source
  environment
...
EvidenceRef
  id
  caseId
  source: coroot | erp | tool | workflow | user | change | log | trace
  entityId
  rawRef
  summary
  confidence
...
- Coroot、Prompt Trace、Workflow、Runbook、Experience Pack、Memory、多用户、多 Agent、ERP 都被纳入统一平台架构。
