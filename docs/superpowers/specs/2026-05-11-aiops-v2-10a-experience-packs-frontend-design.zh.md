...
Experience Pack 自我演化前端不是“经验库 CRUD”，也不是“自动把成功步骤发布为最佳实践”。它是围绕 Evolution Map 和 Experience Pack 的自我演化审计界面，负责让用户看清楚：系统从哪些真实运维事件中生成了什么候选、候选为什么能或不能被激活、如何派生 Runbook / Workflow / 变体、以及已激活资产在后续运行中表现如何。
...
- 审核 Gene、Capsule、Debug RCA、RepairPlan、Runbook draft、Workflow graph draft、Anti-pattern、Incompatibility edge 和 Environment variant。
- 明确候选和已激活资产的隔离边界：candidate 永远不进入 AI Chat 推荐、DebugCase 推荐、中间件修复推荐、正式 Runbook 或正式 Workflow。
- 将审核通过的 Gene/Capsule/Debug RCA/RepairPlan 写入 Activation Index，或将 Runbook/Workflow draft 推入对应发布流程。
...
- 对已发布资产创建环境变体，但不原地修改已发布 Runbook / Workflow / Capsule。
...
- Experience Pack 是演化容器，Gene/Capsule/Debug RCA/RepairPlan/Runbook/Workflow 是可派生资产。
...
- Gene、Capsule、Debug RCA、RepairPlan、Runbook draft、Workflow draft 如何审核和派生。
- 环境变体如何创建、比较、审核、验证。
- Fitness 如何影响 Activation Index 排序、降权、pause 和 deprecated。
- Experience Pack 如何与 AI Chat、Debug Trace、Middleware Repair、Runner Studio、Runbook Catalog、Postmortem 和 Eval 互相跳转。
...
- `web/src/pages/RunnerStudioPage.tsx`：可承接 Workflow graph draft、validate、dry-run、publish 和来源提示。
...
Workflow draft 必须进入 Runner Studio 的 validate / dry-run / publish gate。
...
- Runner Studio 能展示 experience_pack_id、variant_key、review_record_id、evidence chain 和 environment scope。
