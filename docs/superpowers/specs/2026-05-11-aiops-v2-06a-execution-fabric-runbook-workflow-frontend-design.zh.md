...
- `RunnerStudioPage.tsx` 仍有页面内 `requestJson`，应迁移到 `runnerStudioClient` 或统一 Execution Fabric API client。
- Runbook 缺少创建、编辑、版本、review、publish、instance 运行过程、ActionProposal 生成和 verification/rollback 细节。
- Runner Studio 已有 start host groups 和 end callbacks 雏形，但还缺模块设计中的 targetSelector、lockPolicy、fanout、failureThreshold、outputs、callbacks、verification 的稳定页面模型。
- Workflow 运行提交目前更多是 Runner Studio 运行语义，缺少 ActionToken、HostLease、normalizedInputHash、tool schema、audit、EvidenceRef 的统一展示。
...
Workflow Studio 是当前 `RunnerStudioPage` 的企业级目标形态。
...
- `workflow_draft` 跳到 Workflow Studio。
...
- Workflow Studio 支持 Start targetSelector、labels、explicitHosts、variables、lockPolicy、fanout、batchSize、failureThreshold。
...
- Validate、dry-run、risk review、publish gate 在 UI 中清楚可见，未通过不能发布。
