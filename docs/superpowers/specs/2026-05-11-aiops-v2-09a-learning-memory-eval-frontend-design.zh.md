...
- `web/src/pages/RunbookCatalogPage.tsx` 与现有 Runner workflow API/catalog：可承接从候选生成 Runbook draft / Workflow draft；不要求改 Runner 页面。
...
- Artifact Diff：SkillCard/Debug RCA/RepairPlan/Runbook/Workflow/Memory/OpsGraph patch 的 diff；Gene/Capsule 后续实现时再展示。
...
Workflow draft 发布必须走现有 Runner validate / dry-run / publish gate；Learning 页面只展示 gate 结果和 metadata，不要求改 Runner 页面。
...
现有 Runner workflow metadata 写入：
