...
这个模块也不拥有目标资产生命周期。`workflow_draft` 进入 Execution Fabric，`repair_plan_draft` 进入 Middleware Repair，`experience_candidate_summary` 进入 Learning/Experience Pack。AI 只负责生成可审查的类型化输出和证据解释。
...
User: 帮我修复 xxx 的 PG 集群
  ...
  -> if no match: output repair_plan_draft to Middleware Repair
  -> wait for user confirmation
  -> Middleware Repair materializes RepairPlan
  -> Governed Action / Execution Fabric creates ActionProposal or Workflow run
```
