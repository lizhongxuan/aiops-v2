...
5. 调用 `agentmgr.AgentManager.Spawn` 或扩展 `CreateHostAgent`。
6. 创建 child session/thread。
7. 写入 mission store、transcript store。
8. 投影 `childAgents[agentId].status = "spawning"`。
...
11. manager 最终回答汇总三台主机的执行状态、风险和下一步。
12. 高风险动作必须审批。
